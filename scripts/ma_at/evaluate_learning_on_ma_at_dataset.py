#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""在 MA-AT dynamic benchmark 上评估学习方法。"""
...
def parse_args(default_method=None):
    parser = argparse.ArgumentParser(description="Evaluate learning baselines on the MA-AT dynamic benchmark.")
    ...
    parser.add_argument(
        ...
        help="MA-AT dynamic dataset root directory.",
    )
    parser.add_argument(
        "--output-root",
    ...
    parser.add_argument(
        "--config-name",
        default=None,
        help="Only evaluate one MA-AT config, e.g. kn25_ks5_km100.",
    )
    parser.add_argument(
        "--env-file",
        default=None,
...
def evaluate_single_env(env_path, method_bundle, device, arrival_rate, max_total_tasks, simulation_time_limit, seed):
...
def main(default_method=None):
    ...
    print("=" * 80)
    print(f"{method_bundle['display_name']} on MA-AT dynamic benchmark")
    print("=" * 80)
    ...
    log_file.write(f"{method_bundle['display_name']} on MA-AT dynamic benchmark\n")
