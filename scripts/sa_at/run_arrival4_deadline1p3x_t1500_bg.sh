...
#   SA-AT n300_w30_h200_t1500 only
#   methods: Greedy, HRLF, CAPAM, BASAR (ours)
#   arrival_rate = 4.0
#   deadline parameters = 1.3x current t1500 relaxed profile:
#     buffer 20 -> 26, horizon 60 -> 78
# Results are written to an isolated root and do not overwrite paper-table runs.
...
CONFIG="n300_w30_h200_t1500"
ARRIVAL_RATE="4.0"
DEADLINE_BUFFER="26.0"
DEADLINE_HORIZON="78.0"
URGENT_PROBABILITY="0.65"
...
def has_valid_result(method, env_name):
    ...
    return (
        ...
        and abs(float(options.get("dynamic_deadline_buffer", -1.0)) - 26.0) < 1e-9
        and abs(float(options.get("dynamic_deadline_horizon", -1.0)) - 78.0) < 1e-9
    )
...
for method in methods:
    ...
    print(f"{method:6s} {config_name}: n={n} SR={sr:.1f}% DSR={dsr:.1f}% ADV={adv:.2f} FT={ft:.2f} PT={pt:.2f}s")
PY
