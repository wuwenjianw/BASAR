...
OUTPUT_ROOT="${OUTPUT_ROOT:-${REPO_ROOT}/artifacts/results/sa_at_scaling}"
...
echo "SA-AT one-click evaluation"
...
run_eval "Greedy" \
  "${REPO_ROOT}/scripts/sa_at/evaluate_greedy_on_sa_at_dataset.py" \
  "${COMMON_ARGS[@]}"

if has_checkpoint "${HRLF_FOLDER}"; then
  run_eval "HRLF" \
    "${REPO_ROOT}/scripts/sa_at/evaluate_hrlf_on_sa_at_dataset.py" \
...
if has_checkpoint "${CAPAM_FOLDER}"; then
  run_eval "CAPAM" \
    "${REPO_ROOT}/scripts/sa_at/evaluate_capam_on_sa_at_dataset.py" \
...
echo
echo "All requested SA-AT evaluations finished."
